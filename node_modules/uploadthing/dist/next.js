...
var fileCountLimitHit = (files, routeConfig) => {
  ...
  const counts = {};
  files.forEach((file) => {
    const type = (0, import_shared2.getTypeFromFileName)(file, (0, import_shared2.objectKeys)(routeConfig));
    if (!counts[type]) {
      counts[type] = 1;
    } else {
      counts[type] += 1;
...
var uploadFilesInternal = async (data, opts) => {
  ...
  const res = await opts.fetch((0, import_shared3.generateUploadThingURL)("/api/uploadFiles"), {
    method: "POST",
...
var UTApi = class {
  ...
  async requestUploadThing(pathname, body, fallbackErrorMessage) {
    getApiKeyOrThrow();
    const res = await this.fetch((0, import_shared4.generateUploadThingURL)(pathname), {
      method: "POST",
      cache: "no-store",
