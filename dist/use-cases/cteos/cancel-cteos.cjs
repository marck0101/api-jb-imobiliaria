...
var cancel_cteos_exports = {};
__export(cancel_cteos_exports, {
  CancelCteosUseCase: () => CancelCteosUseCase
});
module.exports = __toCommonJS(cancel_cteos_exports);
...
var CancelCteosUseCase = class {
  async execute({ ref, justificativa }) {
    ...
    const response = await import_axios2.default.delete(`${env.FOCUS_NFE_BASE_URL}/cte/${ref}?token=${env.FOCUS_NFE_TOKEN}`, {
      data: { justificativa },
      headers: {
